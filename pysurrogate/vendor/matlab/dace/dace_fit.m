function  [dmodel] = dace_fit(S, Y, i_regr, i_corr, theta0, lob, upb)
...
if  nargin > 5
    ...
    n_var = size(theta0,1);
    lF = zeros(restarts,1);
    lTheta = zeros(restarts,n_var);

    for k = 1:restarts

        % set a random theta value in the bounds if not first run
        if k > 1
            theta0 = lob + rand(n_var,1) .* (upb - lob);
        end

        % Bound constrained non-linear optimization
        [theta f fit perf] = boxmin(theta0, lob, upb, par);

        % save theta and the function value
        lF(k,:) = f;
        lTheta(k,:) = theta;
    end

    % choose the best theta out of all restarts
    [f,I] = min(lF);
    theta = lTheta(I,:);
...
% Return values
dmodel = struct('regr',i_regr, 'corr',i_corr, 'theta',theta.', ...
  'beta',fit.beta, 'gamma',fit.gamma, 'sigma2',sY.^2.*fit.sigma2, ...
  'S',S, 'Ssc',[mS; sS], 'Ysc',[mY; sY], ...
  'C',full(fit.C), 'Ft',fit.Ft, 'G',fit.G);
